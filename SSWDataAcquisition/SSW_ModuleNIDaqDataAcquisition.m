...
saveFolderPath = 'C:\Users\pablo\Desktop\InvestigacionUSFQ\SSWCompleteAnalysis\ResultadosBolazos\CuadriculaIntermedia';  % Replace with the actual folder path
...
rate = 51200;
dq.Rate = rate; % Defines Rate or number of Scans Made per Second
...
while true
    ...
    [data, ~] = read(dq, rate, "OutputFormat", "Matrix"); % Read data for 1 second (1000 scans)
    ...
    if ~isempty(data)
        ...
        % Calculate the number of samples received
        numSamples = size(data, 1);

        % Calculate timestamps based on the sample rate
        sampleRate = dq.Rate;
        timestamps = linspace(elapsedTime - numSamples/sampleRate, elapsedTime, numSamples)';
        ...
        if isvalid(fig) && isvalid(plotAxes)
            ...
            ylim(plotAxes, [min(allData), max(allData)+0.05]);
